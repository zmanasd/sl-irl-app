...
final class SettingsVM: ObservableObject {
    ...
    func diagnosticsSnapshot(
        ...
        Relay User: \(shortIdentifier(settings.relayEffectiveUserId))
        Relay Session: \(settings.hasRelaySession ? "Authenticated" : "Local MVP")
        ...
        App Receipt: \(relayClient.lastAppReceiptStatusCode.map { "HTTP \($0)" } ?? "Not posted")
        App Receipt Correlation: \(relayClient.lastAppReceiptCorrelationId.map(shortIdentifier) ?? "None")
        App Receipt Error: \(relayClient.lastAppReceiptError ?? "None")
